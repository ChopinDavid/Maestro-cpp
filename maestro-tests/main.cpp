...
//Mate in 1, white to move
// Board board = Board::from("2Q2b1k/7p/p5p1/4p3/p5p1/P6P/r4P2/5RK1 w - - 3 38");
// pair<string, int> selectedMove = Search::iterativeDeepeningSearch(board, 5, std::chrono::milliseconds(50000));
// string output = moves.convertMoveToAlgebraUCI(selectedMove.first, board);
// CHECK(output == "c8f8");

// Board board = Board::from("8/RP3pk1/P7/8/7p/7P/1r5r/R4K2 b - - 0 43");
// pair<string, int> selectedMove = Search::iterativeDeepeningSearch(board, 1, std::chrono::milliseconds(6000));
// string output = moves.convertMoveToAlgebraUCI(selectedMove.first, board);
// CHECK(output == "h2h1");
...
TEST_CASE("iterative deepening works")
{
    ...
    Board board = Board::from("r1bqkbnr/2pp1ppp/p3p3/np6/4P3/1B1P1N2/PPP2PPP/RNBQ1RK1 b kq - 0 6");
    pair<string, int> selectedMove = Search::iterativeDeepeningSearch(board, 4, std::chrono::milliseconds(1000000));
    string output = moves.convertMoveToAlgebraUCI(selectedMove.first, board);
    CHECK(output == "a5b3");
...
TEST_CASE("Engine sees underpromotion")
{
    // Moves &moves = Moves::getInstance();
    // Board board = Board::from("8/R4P2/4k3/8/4K3/8/8/8 w - - 1 63");
    // pair<string, int> selectedMove = Search::iterativeDeepeningSearch(board, 5, std::chrono::milliseconds(0));
    // string output = moves.convertMoveToAlgebraUCI(selectedMove.first, board);
    // CHECK(output == "f2f1r");
}

TEST_CASE("Zobrist key generation works") {
    // Board board1 = Board::from("rnbqkb1r/ppp2ppp/3p1n2/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR w KQkq - 0 4");
    // Zobrist zobrist = Zobrist();
    // uint64_t key1 = zobrist.hash(board1);
    // uint64_t key2 = zobrist.hash(board1);
    // CHECK(key1 == key2);

    // Board board2 = Board::from("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 0");
    // uint64_t key3 = zobrist.hash(board2);
    // CHECK(key2 != key3);
}